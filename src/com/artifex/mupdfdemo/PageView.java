...
public abstract class PageView extends ViewGroup {
	...
	private static final int INK_COLOR = 0xFF00008B;
	...
	private float stampscale = 1f;
	private boolean drawingstamp = false;
	private BitmapHolder stampHolder;
	...
	public void drawStampBitmap(Bitmap stamp){
		stampHolder = new BitmapHolder();
		float scale = mSourceScale * (float) getWidth() / (float) mSize.x;
		Log.i("lefttop",getWidth()+"  "+getHeight()+"  "+scale);
		int x=(int)(getWidth()/scale/2);
		int y=(int)(getHeight()/scale/2);
		mStampPointer = new Point(x,y);
		stampscale=1;
		stampHolder.setBm(stamp);
		if(mSearchView!=null){
			mSearchView.invalidate();
		}
	}
	public void stopDrawStamp(){
		stampHolder=null;
	}
	public boolean saveStampBitmap(){
		final float pagescale = mSourceScale*(float)getWidth()/(float)mSize.x;
		HashMap<String,Object>map=new HashMap<String,Object>();
		map.put("stamp",stampHolder);
		map.put("point",mStampPointer);
		map.put("stampscale",stampscale);
		map.put("page", getPage());
		map.put("pagescale", pagescale);
		map.put("ab_y", getHeight()/pagescale-mStampPointer.y);
		stampHolder=null;
		appPDF.ifstampchange=true;
		return appPDF.stampHolderlist.add(map);
	}
		...
		if (mSearchView == null) {
			mSearchView = new View(mContext) {
				...
				protected void onDraw(final Canvas canvas) {
					...
					try {
						for (HashMap<String, Object> stamp : appPDF.stampHolderlist) {
							int page = (Integer) (stamp.get("page"));
							if (page != getPage()) {
								continue;
							}
							Bitmap mBitmap = ((BitmapHolder) stamp.get("stamp"))
									.getBm();
							Point stamppoint = (Point) stamp.get("point");
							float f_stampscale = (Float) stamp
									.get("stampscale");
							float f_pagescale = (Float) stamp
									.get("pagescale");
							Matrix matrix = new Matrix();
							// 缩放原图
							matrix.postScale(scale * f_stampscale/f_pagescale, scale
									* f_stampscale/f_pagescale);
							Bitmap cur_stampbmp = Bitmap.createBitmap(mBitmap,
									0, 0, mBitmap.getWidth(),
									mBitmap.getHeight(), matrix, true);
							int x = (int) (stamppoint.x * scale);
							int y = (int) (stamppoint.y * scale);
							int width = cur_stampbmp.getWidth();
							int height = cur_stampbmp.getHeight();
							Rect mRect = new Rect(x - width / 2,
									y - height / 2, x + width / 2, y + height
											/ 2);
//							paint.setAntiAlias(true);
//							paint.setXfermode(new PorterDuffXfermode(
//									PorterDuff.Mode.SRC_IN));
							canvas.drawBitmap(cur_stampbmp, null, mRect, null);
//							cur_stampbmp.recycle();
						}
						if (drawingstamp && stampHolder != null) {
							Bitmap mBitmap = stampHolder.getBm();
							if (mBitmap == null) {
								return;
							}
							Matrix matrix = new Matrix();
							// 缩放原图
							matrix.postScale(stampscale,stampscale);
							Bitmap cur_stampbmp = Bitmap.createBitmap(mBitmap,
									0, 0, mBitmap.getWidth(),
									mBitmap.getHeight(), matrix, true);
							int x = (int) (mStampPointer.x * scale);
							int y = (int) (mStampPointer.y * scale);
							int width = cur_stampbmp.getWidth();
							int height = cur_stampbmp.getHeight();
							Rect mRect = new Rect(x - width / 2,
									y - height / 2, x + width / 2, y + height
...
//							cur_stampbmp.recycle();
	...
	public void setStampdrawstate(boolean ifdraw){
		drawingstamp=ifdraw;
	}
	...
	public void moveDraw(float x, float y) {
		float scale = mSourceScale * (float) getWidth() / (float) mSize.x;
		float docRelX = x /scale;
		float docRelY = y /scale;
		if (mDrawing != null && mDrawing.size() > 0) {
			for (ArrayList<PointF> drawable : mDrawing) {
				for (PointF point : drawable) {
					point.x += docRelX;
					point.y += docRelY;
				}
			}
	...
	public void moveStamp(float x, float y) {
		float scale = mSourceScale * (float) getWidth() / (float) mSize.x;
		float docRelX = x /scale;
		float docRelY = y /scale;
		if(mStampPointer!=null){
			mStampPointer.x+=docRelX;
			mStampPointer.y+=docRelY;
		}
		mSearchView.invalidate();
	}
	public void scaleStamp(float scale,float x, float y) {
		if(mStampPointer!=null){
			stampscale *= scale;
			if(stampscale<0.2){
				stampscale=0.2f;
			}else if(stampscale>3){
				stampscale=3f;
			}
		}
		mSearchView.invalidate();
	}
	public void scaleDraw(float scale,float x, float y) {
		// TODO Auto-generated method stub
		float mid_scale = mSourceScale * (float) getWidth() / (float) mSize.x;
		float midX = (x - getLeft()) / mid_scale;
		float midY = (y - getTop()) / mid_scale;
		if (mDrawing != null && mDrawing.size() > 0) {
			for (ArrayList<PointF> drawable : mDrawing) {
				for (PointF point : drawable) {
					
					point.x = midX + scale *(point.x - midX);
					point.y = midY + scale *(point.y - midY);
