...
public class MuPDFReaderView extends ReaderView {
	enum Mode {Viewing, Selecting, Drawing , StampDrawing}
	...
	public MuPDFView getCurrentPage(){
		
		return (MuPDFView) getDisplayedView();
	}
	...
	public boolean onTouchEvent(MotionEvent event) {
			...
			case MotionEvent.ACTION_MOVE:
				if (mode == DRAG) {
					...
					if (newDist > 10f) {
						...
						touch_m_move(dx2 / 2, dy2 / 2);
						float scale = newDist / oldDist;
						touch_m_zoom(scale, mid.x, mid.y);
						oldDist = newDist;
						midPoint(mid, event);
					}
				}
		...
		} else if(mMode == Mode.StampDrawing){
			float x = event.getX();
			float y = event.getY();
			switch (event.getAction() & MotionEvent.ACTION_MASK) {
			case MotionEvent.ACTION_DOWN:
	...
	private void touch_m_start(float x, float y) {
		
		mX = x;
		mY = y;
		
	}
	private void move_stamp(float x, float y) {

		float dx = x - mX;
		float dy = y - mY;
		MuPDFView pageView = (MuPDFView) getDisplayedView();
		if (pageView != null) {
			pageView.moveStamp(dx, dy);
		}
		mX = x;
		mY = y;

	}
	private void zoom_stamp(float scale,float x, float y) {

		MuPDFView pageView = (MuPDFView) getDisplayedView();
		if (pageView != null) {
			pageView.scaleStamp(scale,x,y);
		}

	}
	private void touch_m_move(float x, float y) {

		float dx = x - mX;
		float dy = y - mY;
		MuPDFView pageView = (MuPDFView) getDisplayedView();
		if (pageView != null) {
			pageView.moveDraw(dx, dy);
		}
		mX = x;
		mY = y;

	}
	private void touch_m_zoom(float scale,float x, float y) {
		MuPDFView pageView = (MuPDFView) getDisplayedView();
		if (pageView != null) {
			pageView.scaleDraw(scale,x,y);
		}
	}
