...
JNIEXPORT void JNICALL
JNI_FN(MuPDFCore_saveInternal)(JNIEnv * env, jobject thiz , jstring jpath)
{
	globals *glo = get_globals(env, thiz);
	fz_context *ctx = glo->ctx;
	char* savepath = (*env)->GetStringUTFChars(env, jpath, NULL);
	if (glo->doc && glo->current_path && savepath)
	{
		...
		fz_try(ctx)
		{
			fz_write_document(glo->doc, savepath, &opts);
			written = 1;
		}
		fz_catch(ctx)
		{
